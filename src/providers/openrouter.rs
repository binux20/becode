    ...
    fn convert_messages(&self, messages: &[Message]) -> Vec<ApiMessage> {
        messages.iter().map(|msg| {
            ...
            let content = match &msg.content {
                MessageContent::Text(text) => ApiContent::String(text.clone()),
                MessageContent::Parts(parts) => {
                    let api_parts: Vec<ContentItem> = parts.iter().map(|p| match p {
                        ContentPart::Text { text } => ContentItem::Text { text: text.clone() },
                        ContentPart::ImageUrl { image_url } => ContentItem::ImageUrl {
                            image_url: ImageUrlData { url: image_url.url.clone() },
                        },
                    }).collect();
...
impl LLMProvider for OpenRouterProvider {
    ...
    async fn chat(
        ...
        let response = self.client
            ...
            .header("HTTP-Referer", "https://github.com/becode")
            .header("X-Title", "BeCode")
            .json(&request)
            .send()
            .await
